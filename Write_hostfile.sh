#!/bin/bash
GPUS_PER_NODE=4
mkdir -p /pscratch/sd/s/saiedaaz/LLM-Project/QwenTraining/hostfiles
# need to add the current slurm jobid to hostfile name so that we don't add to previous hostfile
hostfile=/pscratch/sd/s/saiedaaz/LLM-Project/QwenTraining/hostfiles/hosts_$SLURM_JOBID
